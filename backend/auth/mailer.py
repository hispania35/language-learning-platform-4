...
def verify_email_letter(name, link):
    """Письмо со ссылкой для подтверждения адреса почты."""
    return _wrap(
        "Подтвердите почту",
        [
            f"{name}, здравствуйте!",
            f"Вы зарегистрировались на платформе {SITE_NAME}.",
            "Нажмите кнопку ниже, чтобы подтвердить адрес и открыть доступ в личный кабинет.",
            "Ссылка действует 24 часа. Если вы не регистрировались — просто удалите письмо.",
        ],
